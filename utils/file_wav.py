...
def help():
	print("read_wav_data(filename) 读取一个wav文件，返回声音信号的时域谱矩阵和播放时间")
	print("GetMfccFeature(wavsignal, fs) 输入波形信号以及频率返回mfcc特征")
	print("GetFrequencyFeature[2,3,4](wavsignal, fs) 输入波形信号以及频率返回频率特征")
	print("wav_scale[2,3](energy) 语音信号能量归一化")
	print("wav_show(wave_data, fs) 输入波形信号以及频率,显示出来声音波形")
